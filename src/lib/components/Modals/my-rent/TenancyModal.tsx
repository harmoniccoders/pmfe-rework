...
const TenancyModal = ({ isOpen, onClose, category, propertyData }: Props) => {
  ...
  const loadReceipt = async () => {
    ...
    try {
      const result = await _dataAccess.get(
        `/api/Property/property/receipt/${propertyData.id}`
      );
      // console.log(result);
      if (result.status) {
        setLoading(false);
  ...
  return (
    <>
              ...
              <Box borderRadius="8px 8px 0 0" overflow="hidden">
                <Flex w="100%" pos="relative" flexDirection="column">
                  <Box w="full" h="250px" pos="relative">
                    <Image
                      src={
                        propertyData.property.mediaFiles?.length > 0
                          ? propertyData.property.mediaFiles[0].url
                          : '/assets/nb.webp'
                      }
                      alt="propery-image"
                      w="100%"
                ...
                >
                  ...
                  <Button
                    ...
                    onClick={loadReceipt}
                    isLoading={load}
                    justifyContent={load ? 'center' : 'flex-start'}
                  >
                    <Box
                      pr="10px"
                      _groupHover={{
                  ...
                  <Button
                    ...
                    alignItems="center"
                    disabled={
                      propertyData?.property?.rentDueDate !== new Date()
                    }
                  >
                    <Box
                      pr="10px"
      ...
      <ReceiptModal open={open} close={close} data={receiptData} />
    </>
  );
};
